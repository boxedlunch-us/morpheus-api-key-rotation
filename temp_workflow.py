...
def execute_rest_no_bearer(method, url, bearertoken, payload):
...
def create_cypher(secret_value, secret_name, appliance_name, token, lease_duration=""):
    if not lease_duration:
        ...
        ttl = lease_duration
    url = "https://" + appliance_name + "/api/cypher/secret/" + secret_name + "?value=" + secret_value + "&type=string&ttl=" + str(ttl)
